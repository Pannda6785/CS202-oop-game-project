...
StormbeastGraphicsComponent::StormbeastGraphicsComponent() : CharacterGraphicsComponent() {
    loadTextures();
}

StormbeastGraphicsComponent::~StormbeastGraphicsComponent() {
    unloadTextures();
}

void StormbeastGraphicsComponent::loadTextures() {
    ...
    std::string character_path = "../assets/sprites/stormbeast/";
    animations["idle"] = Animation(loadAnim(character_path + "movement/idle", 1), 4, true);

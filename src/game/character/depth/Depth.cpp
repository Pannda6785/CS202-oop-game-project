...
#include "handler/DepthBasicHandler.hpp"
#include "handler/DepthWideHandler.hpp"
#include "handler/DepthOffensiveHandler.hpp"
#include "handler/DepthDefensiveHandler.hpp"
...
Depth::Depth() : HandlerCharacter("Depth", 570.0f, 330.0f, std::make_unique<DepthGraphicsComponent>()) {
    ...
    moveHandlers[static_cast<int>(Unit::Move::Basic)] = std::make_unique<DepthBasicHandler>(heroGfx);
    ...
    moveHandlers[static_cast<int>(Unit::Move::Defensive)] = std::make_unique<DepthDefensiveHandler>(heroGfx);
    neutralize();
}
